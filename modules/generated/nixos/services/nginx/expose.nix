...
in {
  options = let
    ...
    host-options = {
      ...
      options = {
        ...
        services =
          builtins.mapAttrs (service: service-cfg: {
            enable = lib.mkOption {
              type = lib.types.bool;
  ...
  services.nginx.virtualHosts = lib.foldlAttrs (res: host-name: host:
    res // (lib.foldlAttrs (acc: service-name: service: let
      service-config =
        self.nixosConfigurations.${host-name}.config.services.http-services.${service-name};
