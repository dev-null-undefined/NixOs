{
  ...
  setLedsScript =
    lib.concatStringsSep "\n" (lib.mapAttrsToList (name: value: "echo ${toString value.brightness} > /sys/class/leds/${name}/brightness")
      cfg.segments);
in {
  options = {
  ...
  systemd.tmpfiles.rules =
    lib.mapAttrsToList (name: value: "w /sys/class/leds/${name}/brightness - - - - ${toString value.brightness}")
    cfg.segments;
