{
  ...
  # str -> [ str ]
  pathToModuleParts = path:
    pathSplit (lib.strings.removeSuffix ".nix" (builtins.toString path));
  # Split file path and remove the common prefix (that being the path to the mainDir
  # and then prepend the prefix part
  # str -> [ str ]
  removeCommonPart = parts:
    prefix
    ++ lib.lists.drop (builtins.length (pathToModuleParts mainDir)) parts;
  ...
  nixKeyPathsToFolders = keyPaths:
    lib.lists.unique (builtins.concatMap (keyPath:
  ...
  mkFolderOption = keyPath:
    {
      enable = lib.mkOption {
        ...
        default = false;
        description = ''
          This option is autogenerated option from folder containing nix files. Enabling this option will enable these subfiles:
          ${let
            enabled =
              lib.attrsets.filterAttrsRecursive (n: _v: n != "enable")
              (generateFolderEnableAttrs keyPath.path);
          in
      ...
      all.enable = lib.mkOption {
        ...
        description = ''
          ...
          ${let
            enabled =
              lib.attrsets.filterAttrsRecursive (n: _v: n != "enable")
              (generateFolderEnableAllAttrs keyPath.path);
          in
            lib.strings.concatMapStrings (option: " - " + option + "\n")
            (lib'.internal.getAttrsPaths enabled)}'';
      };
  ...
  generateConfig = {
    ...
    config =
      ...
      (lib.attrsets.filterAttrs (n: _v: !builtins.elem n filtered)
        (getConfigFromFile keyPath.path));
  };
  ...
  # str -> {}
  generateFolderEnableAttrs = folder:
    filterEnabledIfDefault (enableAllSubOptions (getNixKeyPaths folder));

  folderHasDefault = folder:
    builtins.elem default
    (builtins.map (nixKeyPath: lib.lists.last nixKeyPath.parts)
      (getNixKeyPaths folder));
  ...
  generateFolderConfig = folder:
    [
      ({config, ...}: {
        ...
        config =
          lib.mkIf
          (lib.attrsets.attrByPath (folder.parts ++ ["enable"]) false config)
          (generateFolderEnableAttrs folder.path);
      })
    ]
    ++ (lib.lists.optional (folderHasDefault folder.path) ({config, ...}: {
      _file = folder.path;
      config =
        lib.mkIf
        (lib.attrsets.attrByPath (folder.parts ++ ["all" "enable"]) false
          config) (generateFolderEnableAllAttrs folder.path);
    }));
...
in {
  options = lib.attrsets.recursiveUpdate (builtins.foldl'
    (acc: keyPath: lib.attrsets.recursiveUpdate (generateOption keyPath) acc)
    {}
    nixKeyPaths) (builtins.foldl' (acc: keyPath:
    lib.attrsets.recursiveUpdate (generateFolderOption keyPath) acc) {}
  foldersKeyPaths);
  imports =
    (builtins.map (keyPath: generateConfig {inherit keyPath;}) nixKeyPaths)
    ++ (builtins.concatMap generateFolderConfig foldersKeyPaths);
}
