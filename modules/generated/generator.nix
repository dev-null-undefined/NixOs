...
let
  ...
  # str -> [ str ]
  removeCommonPart =
    parts: prefix ++ lib.lists.drop (builtins.length (pathToModuleParts mainDir)) parts;
    ...
    keyPath:
    {
      enable = lib.mkOption {
        ...
        description = "This option is autogenerated option from folder containing nix files. Enabling this option will enable these subfiles:\n${
          let
            enabled = lib.attrsets.filterAttrsRecursive (n: _v: n != "enable") (
  ...
  generateOption =
    keyPath:
    if (builtins.length keyPath.parts) == 0 then
  ...
  getConfigFromFile =
    ...
    in
    if isFunction then
      configContent ({ osConfig = { }; } // outerArgs // { inherit lib'; })
    else
      configContent;
  ...
  filterEnabledIfDefault =
    attrset:
    if isOverride attrset then
      attrset
    else
      (
        if lib.attrsets.hasAttrByPath [ default ] attrset then
          { inherit (attrset) default; }
        else
          lib.attrsets.mapAttrs (_n: filterEnabledIfDefault) attrset
      );
  ...
  # str -> {}
  generateFolderEnableAttrs =
    folder: filterEnabledIfDefault (enableAllSubOptions (getNixKeyPaths folder));
  ...
  generateFolderConfig =
    ...
    [
      (
        ...
        {
          _file = folder.path;
          config = lib.mkIf (lib.attrsets.attrByPath (folder.parts ++ [ "enable" ]) false config) (
    ...
    ++ (lib.lists.optional (folderHasDefault folder.path) (
      ...
      {
        _file = folder.path;
        config = lib.mkIf (lib.attrsets.attrByPath (
          folder.parts
          ++ [
            "all"
            "enable"
          ]
        ) false config) (generateFolderEnableAllAttrs folder.path);
      }
    ));
...
in
{
  options =
    lib.attrsets.recursiveUpdate
      ...
      (
        builtins.foldl' (
          acc: keyPath: lib.attrsets.recursiveUpdate (generateFolderOption keyPath) acc
        ) { } foldersKeyPaths
      );
  imports =
