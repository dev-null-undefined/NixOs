...
Modules is split into 4 sub folders `nixos`, `home-manager`, `darwin` and `generated`.
...
Contains the darwin (macOS) modules. Same structure as `modules/nixos/`. Used by `nix-darwin` hosts.
...
<span id="home-nixosDefaults"></span>
...
Packages can be pinned to a different nixpkgs channel by adding them to the `stable-pkgs` or `master-pkgs` overlay in `overlays/default.nix`. This transparently overrides them system-wide — no per-host changes needed.

## `pkgs/`

Custom package derivations. These are made available via the `custom-packages` overlay. See individual package directories for details.

## `secrets/`

Encrypted secrets managed by [SOPS](https://github.com/getsops/sops) with age encryption. Keys are per-host, configured in `.sops.yaml`. Encrypted files live in `secrets/`.

## Deploy-rs

Remote deployment is configured via [deploy-rs](https://github.com/serokell/deploy-rs). Nodes are auto-generated from the `hosts/` directory. Builds happen remotely (`remoteBuild = true`).
